...
class ConvFrelu(nn.Module):
    """Applies a convolution, batch normalization, and activation function to an input tensor in a neural network."""

    default_act = FReLU  # default activation
...
# COORD ATTENTION [✓]
# --------------------------------
# from https://paperswithcode.com/paper/coordinate-attention-for-efficient-mobile#code / https://github.com/houqb/CoordAttention/blob/main/mbv2_ca.py
...
class CoordAtt(nn.Module):
    def __init__(self, inp, oup, groups=32):
        ...
        assert inp == oup, f"Input channels ({inp}) must equal output channels ({oup})"
...
# --------------------------------
# MODIFY YOLO MODEL WITH INHERITED CLASS
# --------------------------------y

class C3WS(nn.Module):
    """CSP Bottleneck with three convolutions, enhanced with Weight Standardization, FReLU, and Coordinate Attention."""
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
    ...
    def forward(self, x, skip_features=None):
        ...
        basic_output = self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1))
        #print("C3WS forward, skip features:", skip_features)
        # Add skip connection if provided
        self.coord_att(basic_output)
        if skip_features is not None:
            #print("adding skip connection")
            basic_output += skip_features  # Additive skip connection
