...
if comet_ml:
    try:
        exp = comet_ml.OfflineExperiment(offline_directory="./comet_offline")
    except comet_ml.CometOfflineException:
        exp = None
...
from custom.loss import CustomComputeLoss
from custom.model import YOLOWS
...
def train(hyp, opt, device, callbacks):
    ...
    init_seeds(opt.seed + 1 + RANK, deterministic=False) # non deterministic, TODO: k-cross
    ...
    if pretrained:
        ...
        model = YOLOWS(cfg or ckpt["model"].yaml, ch=3, nc=nc, anchors=hyp.get("anchors")).to(device)  # create
        ...
        model = YOLOWS(cfg, ch=3, nc=nc, anchors=hyp.get("anchors")).to(device)  # create
    ...
    compute_loss = CustomComputeLoss(model)  # init loss class
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and comet_ml and not check_comet_resume(opt) and not opt.evolve:
