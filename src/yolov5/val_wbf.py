...
def process_batch_wbf(detections, labels, iouv):
    """Similar to process_batch but adapted for WBF outputs"""
    correct = np.zeros((detections.shape[0], iouv.shape[0])).astype(bool)
    iou = box_iou(labels[:, 1:], detections[:, :4])
    correct_class = labels[:, 0:1] == detections[:, 5]
    for i in range(len(iouv)):
...
def apply_wbf(prediction, conf_thres=0.25, iou_thres=0.45, skip_box_thr=0.0001, img_size=640):
    """Apply Weighted Box Fusion instead of NMS"""
    ...
    for xi, x in enumerate(prediction):
        ...
        scores = x[:, 4:5] * x[:, 5:].max(1)[0].unsqueeze(1)  # conf = obj_conf * cls_conf
        classes = x[:, 5:].argmax(1)
        
        # Convert to numpy and normalize to [0, 1]
        boxes_np = box.cpu().numpy() / img_size
        ...
        if len(boxes_wbf):
            out = torch.zeros((len(boxes_wbf), 6), device=device)
            out[:, :4] = torch.from_numpy(boxes_wbf).to(device)
            out[:, 4] = torch.from_numpy(scores_wbf).to(device)
            out[:, 5] = torch.from_numpy(labels_wbf).to(device)
            output[xi] = out

    return output

@smart_inference_mode()
def run_validation_wbf(
    data,
    weights=None,
    batch_size=32,
    imgsz=640,
